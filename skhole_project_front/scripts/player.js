...
let thumbnail = '';
let videoId = '';
const viewSpan = document.getElementById('views');

document.addEventListener('DOMContentLoaded', () => {
    // Retrieve values from localStorage
    videoUrl = localStorage.getItem('selectedVideoUrl');
    thumbnail = localStorage.getItem('selectedThumbnail');
    videoId = localStorage.getItem('selectedId');

    // Check if videoUrl and thumbnail exist
    if (videoUrl && thumbnail) {
        ...
        thumbnailElement.src = `data:image/jpeg;base64,${thumbnail}`;

        // Show the video player row and hide the thumbnail
